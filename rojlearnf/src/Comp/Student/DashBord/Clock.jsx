...
const Clock = () => {
    ...
    return (
        <>       
            <div className="before:absolute before:w-12 before:h-12 before:bg-orange-800 before:rounded-full before:blur-xl before:top-16 relative mx-auto   flex flex-col justify-around items-center w-44 h-44 rounded-2xl shadow-inner shadow-gray-50 bg-neutral-900 text-gray-50">
                <span className="">{day}, {monghts[currDate.split("/")[0] - 1]} {currDate.split("/")[1]}</span>
                <span className="z-10 flex items-center text-6xl text-amber-600 [text-shadow:_2px_2px_#fff,_1px_2px_#fff]">{currTime.split(":")[0]}<span className="text-xl font-bold text-gray-50 [text-shadow:none]">:</span>{currTime.split(":")[1]}</span>
                <div className="text-gray-50 w-48 flex flex-row justify-evenly">
                    <span className="text-xs font-bold">{currTime.split(":")[2].split(" ")[1]}</span>
